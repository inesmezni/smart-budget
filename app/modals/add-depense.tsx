...
import { NouvelleDepense } from '../../src/models';

const formatDateInput = (date: Date) => date.toISOString().slice(0, 10);

export default function AddDepense() {
  const router = useRouter();
  const { ajouter } = useDepenseStore();
  ...
  const validerEtEnvoyer = async () => {
    ...
    const nouvelleDepense: NouvelleDepense = {
    ...
    try {
      await ajouter(nouvelleDepense);
      router.back();
    } catch (error) {
      console.error('Erreur ajout dépense', error);
      Alert.alert('Erreur', 'Impossible d’ajouter la dépense pour le moment.');
    } finally {
  ...
  return (
    ...
    >
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.chipScroll}>
          {isLoading ? (
            ...
            categories.map((cat) => (
              ...
              >
                <Text style={[styles.chipText, categorieId === cat.id && styles.chipTextActive]}>
                  {cat.icone}
                </Text>
                <Text style={[styles.chipLabel, categorieId === cat.id && styles.chipTextActive]}>
                  {cat.nom}
                </Text>
              </TouchableOpacity>
            ))
        ...
        >
          <Text style={styles.buttonText}>{isSubmitting ? 'Enregistrement...' : 'Ajouter la dépense'}</Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  content: { padding: 20 },
  title: { fontSize: 24, fontWeight: '700', color: '#185FA5', marginBottom: 24 },
  valueCard: {
    padding: 20,
    borderRadius: 18,
  ...
  linkText: { color: '#185FA5', fontSize: 14, fontWeight: '600' },
});